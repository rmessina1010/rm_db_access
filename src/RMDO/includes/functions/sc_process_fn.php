...
function apply_sc($str, $obj = NULL, array $xtra = array())
{
	...
	if ($useKey) {
		if (is_array($obj)) {
			if (isset($obj[$str]) && is_scalar($obj[$str])) {
				$str = $obj[$str];
			} else {
				return false;
			}
	...
	if (count($codez[0])) {
		...
		foreach ($ckey as $k) {
			$fn = $dn = '';
			if ($docName[$k]) {
				$dn = substr(trim($docName[$k]), 0, -1);
				$fn = $dn . '_';
			}
			$fooName = 'RMCSC_' . $fn . trim($code[$k]);
			if (!function_exists($fooName)) { // if  function not available, attempt  to load function
				if ($dn && file_exists($path . $dn . '.php')) {
					include_once($path . $dn . '.php');
				} elseif (file_exists($path . $fooName . '.php')) {
					include_once($path . $fooName . '.php');
				} elseif (file_exists($path . SHORTCODES)) {
					include_once($path . SHORTCODES);
				}
			}
